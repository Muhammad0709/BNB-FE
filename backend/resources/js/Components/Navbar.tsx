          ...
          <Stack direction="row" spacing={5} sx={{ display: { xs: 'none', md: 'flex' }, mx: 'auto' }}>
            {links.map((l) => (
              <Typography
                ...
                sx={{ color: isActive(l.href) ? '#FF385C' : '#222222', fontWeight: 700, fontSize: '0.875rem' }}
              >
                {l.label}
              </Typography>
            ))}
          ...
          {showAuth && (
            <Stack direction="row" spacing={2} sx={{ display: { xs: 'none', md: 'flex' }, ml: 'auto' }} alignItems="center">
              {isAuthenticated ? (
                ...
                <>
                  <Typography component={Link} href="/login" sx={{ textDecoration: 'none', color: '#222222', fontWeight: 700, fontSize: '0.875rem' }}>Log in</Typography>
        ...
        <Box sx={{ position: 'fixed', inset: 0, bgcolor: 'rgba(0,0,0,0.4)', zIndex: 1200 }} onClick={() => setOpen(false)}>
          <Box sx={{ position: 'absolute', top: 0, right: 0, width: '80%', maxWidth: 320, height: '100%', bgcolor: '#fff', p: 3 }} onClick={(e) => e.stopPropagation()}>
            ...
            <Stack spacing={2.5} sx={{ mb: 3 }}>
              {links.map((l) => (
                <Typography key={l.label} component={Link} href={l.href} onClick={() => setOpen(false)} sx={{ textDecoration: 'none', color: isActive(l.href) ? '#FF385C' : '#222222', fontWeight: 700, fontSize: '0.875rem' }}>
                  {l.label}
                </Typography>
              ))}
            </Stack>
            {showAuth && (
              <Stack direction="row" spacing={1.5}>
                {isAuthenticated ? (
                  ...
                  <>
                    <Button component={Link} href="/login" variant="text" onClick={() => setOpen(false)} sx={{ textTransform: 'none', fontWeight: 700, fontSize: '0.875rem' }}>Log in</Button>
